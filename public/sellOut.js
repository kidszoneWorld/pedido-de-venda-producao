...
function adicionarLinha(dados = {}) {
    ...
    const campos = [
        'cod_item', 'sku', 'sellIn', 'bairro', 'cidade', 'uf', 
...
async function removerLinha(row, id) {
    if (id) {
        try {
            const response = await fetch('/api/sellOut/remover', {
                method: 'DELETE',
...
async function buscarDadosSellOut(codgroup) {
    try {
        ...
        sellOut.forEach(dados => adicionarLinha(dados));
    } catch (error) {
...
async function salvarDados() {
    ...
    const dados = linhas.map(row => {
        ...
        return {
            ...
            nome,
            cod_item: inputs[0].value,
            sku: inputs[1].value,
            sellIn: inputs[2].value,
    ...
    try {
        const response = await fetch('/api/sellOut/salvar', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ codigo_cliente: codgroup, nome, sellOut: dados })
        });
